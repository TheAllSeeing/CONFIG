...
#    killprev = json.loads(subprocess.check_output(["killall", "termux-wifi-connectioninfo"], universal_newlines=True, timeout=5))
    ...
    if wlan["supplicant_state"] == "COMPLETED":
        ...
        wifi_str = strengthcol + wifiicon + wlan["ssid"] + " @ " +" " + str(wlan["link_speed_mbps"]) + "mbps " + term.normal
        print(wifi_str)
    else: 
        ...
        if data["data_state"] == "connected":
         #   print(data)
            ...
            if data["network_type"] == 'lte':
            	network_type = '4G
            else if data["network_type"] == 'wcdma':
            	network_type = '3G'
            else:
            	network_type = data["network_type"]
            print(dataicon + ' ' + network_type + ' (' + data["network_operator_name"] + ') ' + activity_icon)
        else:
            print(nodataicon + ' Disconnected')
except:
    print(nodataicon + ' getting data...')
    # TODO show last data instead of useless error msg
exit()
